...
export default function GoogleManualAd() {
    ...
    <div className="flex min-h-screen bg-white relative">
      ...
      <div className="flex-1 px-12 py-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold">Create Google Ad Manually</h1>
            <p className="text-sm text-gray-500">Build your Search, Display, or Performance Max campaign with full control</p>
          </div>
          <div className="flex items-center gap-4">
            <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm">🟢 Google Ads</span>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" className="hidden" onChange={(e) => setAiOpen(e.target.checked)} />
              <span className="relative inline-block w-10 h-5 bg-gray-300 rounded-full">
                <span className={`absolute left-1 top-1 w-3 h-3 bg-white rounded-full shadow transition-transform ${aiOpen ? 'translate-x-5' : ''}`}></span>
              </span>
        ...
        <div className="space-y-8">
          <div className="rounded-xl border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <label className="flex gap-2 items-center"><input type="checkbox" /> Include Google search partners</label>
              <label className="flex gap-2 items-center"><input type="checkbox" /> Include Display Network</label>
            </div>
            <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="flex justify-end gap-4">
            <button className="px-6 py-2 rounded border border-gray-300">Save as Draft</button>
            <button className="px-6 py-2 rounded bg-green-600 text-white">Launch Campaign</button>
          </div>
        </div>
      </div>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
    <div className="bg-white p-8 rounded-lg shadow-md w-[360px]">
      <h2 className="text-2xl font-semibold mb-4 text-center">AI Context for Google Ads</h2>
      <p className="text-sm text-gray-500 mb-6 text-center">To generate more relevant Google ad suggestions, please provide some context about your campaign.</p>
      <input
        className="input mb-4 px-4 py-2 border border-gray-300 rounded w-full"
      ...
      />
      <select
        className="input mb-6 px-4 py-2 border border-gray-300 rounded w-full"
        value={aiInput.tone}
      ...
      <button
        className="bg-black text-white w-full py-3 rounded hover:bg-gray-800 transition duration-300"
